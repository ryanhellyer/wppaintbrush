 ...
 * @since 0.1
 */
require( get_stylesheet_directory() . '/legacy.php' ); // Loading support for legacy code
...
function wppb_inline_page_stats() {
	echo "\n<!-- Number of queries = " . get_num_queries() . ". Time to execute = "; timer_stop(1); echo " -->\n";
}
...
function wppb_template_load() {
	$template = get_wppb_option( wppb_template_choice() ); // Load appropriate template
	$template = str_replace( '[get_header]', get_wppb_option( 'header' ), $template ); 
	$template = str_replace( '[get_footer]', get_wppb_option( 'footer' ), $template );
	echo $template;
}
...
eval( wppb_functions_dot_php() ); // Execute virtual functions.php file
