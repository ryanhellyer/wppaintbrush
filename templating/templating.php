...
/**
 * Grabbing @font-face declarations
 * Places @font-face declarations into an array
 * @since 0.1
 */
if ( !function_exists( 'pixopoint_fontface' ) ) :
function pixopoint_fontface( $css, $start, $end ) {
	$font_face = explode( $start, $css );
	$count = 0;
	$the_fonts = array();
	foreach ( $font_face as $bla ) {
		$font = explode( $end, $font_face[$count] );
		if ( 0 < $count )
			array_push( $the_fonts, $font[0] );
		$count ++;
	}
	return $the_fonts;
}
